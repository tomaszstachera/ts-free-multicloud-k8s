...
- build a Kubernetes cluster which will consist of
  - master node on AWS EC2 ARM instance
  - worker nodes on GCP and OCI (Oracle Cloud Infrastructure) x86/64 VMs
- we will only use free tier of cloud resources and fully automate everything in a one-click Terraform deployment
- we will ask DeepSeekAI for the whole code, point out it's mistakes and fix them
...
Apparently 1 vCPU is not enough for master node, but I've found out that AWS has an offer of 2 vCPU and 2 GB RAM ARM Graviton2 offer:

![AWS Graviton2](img/graviton.png "AWS Graviton2")

It turned out that this is enough and cluster bootstrapped successfully with 2 worker nodes:

![Worked with Graviton2](img/worked-manually.png "Worked with Graviton2")

Taking a look at `htop` will show that master is can handle the load with ~ 80% cpu consumed (at the begining):
...
Deepseek is viral recently, so let's check if it can replace DevOps engineers by asking him to prepare the whole code for us.

![Deepseek](img/deepseek.png "Deepseek")

This is the exact prompt that I've used (I don't have Masters Degree in Prompt Engineering ;) so I've only spend few minutes fixing the prompt):
...
Original code that Deepseek generated can be found [here](./original-deepseek-infra/). In general code had a lot of bugs. Some of them were discovered by my Neovim LSP, others appeared during `plan` or `apply`. Let's take a look at them.
...
- in general LSP suggested a lot of useful improvements not added by Deepseek
  - versioning of Providers and TF version
  - variable types
  - others

- Deepseek didn't understand that kubeconfig must be taken from remote server and tried locally
...
On GCP I got 8PLN from networking and comute. Apparently 
